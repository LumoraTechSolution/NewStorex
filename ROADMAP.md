...
- [x] **M3-15** Low stock report — a per-product reorder point, and the one report that is about tomorrow <sub>done 2026-08-30</sub> — `V120` (desktop), `V211` (cloud); `products.reorder_point` is **nullable with no default**, because NULL ("not watched") and 0 ("tell me when it is empty") are different instructions and a shopkeeper's most-watched line is exactly the one that would be lost by folding them together. Still no stored level: low is `qty_on_hand <= reorder_point` against M3-07's view, evaluated when asked. An import never sets one and never clears one — see §G.
...
- [x] **M4-10** Deploy console to Vercel; host the cloud backend <sub>done 2026-08-29</sub> — **Neon** (`lumora_cloud`, ap-southeast-1) + **Render** (`storex-cloud.onrender.com`, Docker, free) + **Vercel**, all Singapore. Shipped as an _image plus an environment contract_ rather than an account on a host, so a client demanding AWS or Azure is a change of values: the Dockerfile, the Java, all eleven cloud migrations and `application-cloud.yml` are identical on ECS or ACA. Neon over Render Postgres because the latter is **deleted after 30 days** and pilot sales cannot live somewhere with an expiry date. Six deploys failed first and **five were bad diagnosis, not bad infrastructure** — see §G, which is worth reading before touching Neon again
...
>
> - [x] **GATE-M4** executed and passed <sub>2026-08-29</sub> — a phone on mobile data, not the shop's wifi, showing today's takings from the cloud

---

### M6 · The till learns to be touched <sub>~1 week</sub>

Inserted before M5 deliberately. The installer packages a UI, and a pilot shop that learns a keyboard-only till and gets retrained a release later is a support call against an appliance whose whole premise is that it does not change under you.

**The rule the whole milestone rests on:** no touch control introduces a new code path. Every button runs the same callback its function key runs, so the keyboard path cannot regress — touch does not exist on it. Gate M1 stays achievable, and every rail button carries its F-key so the two input methods teach each other.

**No product-button grid, by decision.** A shop's catalogue does not fit on a screen, the gun is faster for anything with a barcode, and F3 search covers the rest. That decision removed the entire backend half of this milestone: `ProductSummary`, `ProductLookup` and `ProductController` are untouched, no endpoint was added and no migration was needed.

- [x] **M6-01** `handEvents()` — the e2e pointer predicate that survives touch <sub>done 2026-08-29</sub> — the migration `keyboard-only-sale.spec.ts` documented in M1-16 and left for whoever needed it; that clause has now fired. Landed alone, with no app code, so the suite passing identically under the looser predicate is provable
- [x] **M6-02** Two-column shell — totals beneath the receipt, action rail on the right, F-key bar at 84px <sub>done 2026-08-29</sub> — layout only; the whole suite had to pass untouched, and did
- [x] **M6-03** Cart tap-to-select <sub>done 2026-08-29</sub> — wired the `setSelected` that had existed unused since M1-10
- [x] **M6-04** F3 search overlay <sub>done 2026-08-29</sub> — fills a slot dead since M1-07, and is the **only** way a finger reaches a product with a damaged barcode or none at all
- [x] **M6-05** Action rail + touch keypad — quantity multiplier, Search/Void/Clear, PAY <sub>done 2026-08-29</sub> — digits are touch-only on purpose; see §G
- [x] **M6-06** Tender overlay by touch — payment-type buttons, keypad, quick-cash denominations <sub>done 2026-08-29</sub> — the kind chips were `<span>`s and are now buttons
- [x] **M6-07** F1 help — the key map <sub>done 2026-08-29</sub> — at 84px a dead slot reads as a broken button, not a dim hint
- [-] **M6-08** Backend: categories on the till's catalogue <sub>dropped — no product grid, so the till reads no categories</sub>
- [-] **M6-09** Category strip + product grid <sub>dropped — see the decision above</sub>

**Extended past its own title, deliberately.** `M6-10` onward are not about touch — they are the owner-console work agreed on 2026-08-30 and 2026-09-01. Task IDs are stable and are never renumbered, so a week of console work appends here rather than opening an M7 for it. The extra _reports_ from that same conversation (dead stock, sales by hour, refunds and voids) were **declined on 2026-09-01** — the pilot decides what a seventh tab should be, not us.

- [x] **M6-10** Attention feed — acknowledge a variance <sub>done 2026-09-01</sub> — a variance now **opens**, showing the denominations behind the count that `shift_counts` has carried since `V203` and nothing had ever read, and can be marked reviewed with a note. Reviewed is **not gone**: a second list keeps every cleared variance with the name of whoever cleared it, because a list you can empty and never look into again is a way to hide a shortfall from the person it belongs to. `V214` is a **table, not a boolean on `shifts`** — the shift is a synced aggregate and the next delivery would silently un-review it. This is also the **first and only write a console session may do**, a deliberate widening recorded in `AuthenticatedPrincipal`: it touches no money, is attributed, and cannot hide the shift. 10 tests. See §G
- [x] **M6-11** Shift history that does not end — date range and "load more" <sub>done 2026-09-01</sub> — a range and a button, and **deliberately not page numbers**: a shopkeeper asks for "last week" and for "more", which are two controls, where page three means something different tomorrow on a list that only grows at one end. The cursor is **keyset on `(closed_at, id)`, not `OFFSET`** — a shift closing on the other till mid-read pushes every row down one, and an offset then repeats the row at the page boundary, which is a shift counted twice in a list somebody is reconciling against a drawer. The id is in the tuple because two tills can close in the same microsecond. Half a cursor is treated as none, since a lone timestamp compares against NULL and returns nothing that looks exactly like "no more shifts". 7 tests. See §G
- [x] **M6-12** Stock in the owner console — the missing half of a screen designed with two halves <sub>done 2026-09-01</sub> — `ConsoleReportService` had admitted the gap in a comment since M4-07. Two questions and one of them is urgent: **running out** (ordered by the gap below the threshold, not the level, so it matches the till's own M3-15 ordering) and **look something up** (a search box, because you arrive already knowing what you want to check). On hand is **Σ movements** on the cloud exactly as on the till — a cached level would be a second source of truth, and the drift would surface as an owner and a shopkeeper reading different numbers off two screens. **NULL and 0 stay different instructions**: an unwatched product is never listed however low it goes, and the footer says so on screen. A negative on hand is shown as "not received" rather than "low", because that sends somebody to the goods-in book instead of to a supplier. 11 tests. See §G
...
- [x] **M5-01** Bundle JRE + Postgres 16 + backend jar + Next.js standalone into the Electron app <sub>done 2026-08-30</sub> — `pnpm stage:runtime` builds a 44 MB JRE with **jlink** (not a downloaded Temurin) and prunes Postgres 16.6 to 102 MB; `pnpm dist` assembles all four parts. The Electron main process now starts the whole stack — database, backend, renderer — and stops it in reverse. Three failure modes only running it could have found; see §G
- [x] **M5-02** NSIS single installer <sub>done 2026-08-30</sub> — **178 MB `StoreX-Setup-0.0.0.exe`**, installed and launched on this machine: full stack up in **20 s** on a first run including `initdb`, **11 s** after. A silent uninstall keeps the shop's data, which is asserted rather than assumed
- [x] **M5-03** First-run wizard — tenant seed, activation, key provisioning, branch/terminal codes, invoice block allocation <sub>done 2026-08-31</sub> — four steps (shop, till, owner, cloud) behind an unauthenticated `/api/setup` whose credential is **the empty database**, the same guard `PlatformBootstrap` uses. One transaction: tenant, branch, settings, OWNER user and both invoice blocks, or none of it. The five hardcoded constants in `page.tsx` are gone — a till now reads its own identity. The cloud token is pasted once and stored in `runtime.json` beside the database password, not `setx /M`. **`V121`'s `NOT NULL` broke ten existing tests** and the fix is the interesting part; see §G **Revised 2026-09-01 after a real onboarding failed:** the cloud step now requires an address alongside the token and **verifies the pair against the cloud before anything is written**, naming the shop it belongs to. A token with no address had fallen back to the till's own loopback — sold perfectly, queued 62 rows going nowhere.
- [x] **M5-04** Scheduled local backup (`pg_dump` to a second location) <sub>done 2026-09-01</sub> — twice a day rather than nightly, because a nightly backup is one a shop that switches the PC off never takes. Writes to `LUMORA_BACKUP_DIR` or a folder beside the data, keeps 30 days, and every archive is read back with `pg_restore --list` before it counts as taken. `pg_dump.exe` was already in M5-01's pruned runtime, so nothing new ships
- [~] **M5-05** Restore path, actually tested — a dead disk must not mean lost history <sub>in progress</sub> — `docs/restore.md` is written and the dump→drop→restore cycle is asserted against real Postgres on every `pnpm test`. **The document itself is unproven**: M5-05 says _actually tested_, and that means a person following it on an installed till. Not done until somebody has
- [x] **M5-06** **Automatic cloud backup** — resolves **D5**; treated as a v1 requirement because local-first puts a shop's entire history on one disk <sub>done 2026-09-01</sub> — the till uploads its verified `pg_dump` to the cloud **once a day rather than twice**, because the local copy is free and this one crosses a shop's mobile data. `POST /api/sync/backup` takes the raw archive with its own till credential, streams it to object storage without ever holding it in memory, and **recomputes the SHA-256 from what arrived** — a mismatch deletes what was written, because an archive nobody can vouch for is exactly the one somebody would restore from. Object first, row second, always: there is never a row promising an archive that is not there. Cloudflare R2, chosen on 2026-09-01 over a paid Render disk and over Neon, because **the deployed filesystem is wiped on every release** and a backup that evaporates is worse than none. `V212`; 11 tests over HTTP, including five hostile file names and the cross-tenant assertion. See §G
- [x] **M5-07** Crash recovery — kill the process between writes; assert no partial sale and no orphan movement <sub>done 2026-09-01</sub> — a crash between two writes _is_ a transaction that never committed, so the failure is injected with a database trigger where a disk error would fire rather than by killing a JVM at an instant the scheduler picks. Four tests: no orphan header, no stock movement for goods that never left, **no burned invoice number**, and a till that sells normally afterwards
- [x] **M5-08** Mid-sale power cut must not corrupt a shift <sub>done 2026-09-01</sub> — three tests: a crashed sale leaves the shift open with its counted float intact, an interrupted _close_ leaves it OPEN rather than half-closed (which `ux_shifts_one_open_per_terminal` would make unrecoverable), and such a shift can still be closed with its sales still attached
...
- [x] **M5-10** PDPA — per-customer data export and erasure <sub>done 2026-09-01</sub> — **erasure is anonymisation, and that is the correct answer rather than a compromise**: a shop must keep its financial records, so the personal fields are overwritten and the row keeps its id and `client_uuid`, which means every sale still points at the same now-anonymous party and a Z-report printed last week still reconciles. The export carries **line items, not a total** — the right is to the data. The blanked row is enqueued for the cloud **in the same transaction**, because a shop must not be able to say yes while the number is still up there. An issued **tax invoice keeps what it was printed with** (Gazette 2481/22, and the buyer filed it), and the erasure reports how many there are — nearly always zero. `V123` desktop, `V213` cloud; `docs/pdpa.md` is what a shopkeeper reads to whoever asked. 11 tests. See §G
- [~~] **M5-11** Desktop auto-update channel <sub>built 2026-09-01, deliberately off</sub> — `electron-updater` against a generic https feed, and the design is what it **refuses** to do: `autoInstallOnAppQuit`, `quitAndInstall()` never called, and **no button anywhere in the renderer or the bridge**, so there is no code path that can stop a till mid-shift. An update lands when the shop closes StoreX. The channel **refuses to run on a build that is not code-signed** — without a publisher name there is nothing to check a downloaded installer against, which makes an update channel remote code execution on every shop PC. Left `[~]` rather than ticked for exactly that reason: **it has never run.** Needs an OV certificate (~~$200–400/yr) and a static https host; `docs/releasing.md` is the procedure and the list of what is missing. 10 tests on the decision itself
...
| PDPA No. 9 of 2022                                          | phasing in                         | Export and erasure **done 2026-09-01** (`docs/pdpa.md`). **Breach notification is not software** and stays the shop owner's process. Penalties to Rs. 10M per instance            | M5-10    |
...
| **D5** | Backup strategy             | **Settled 2026-09-01** — three copies: `pgdata`, a local `pg_dump` twice a day (M5-04), and a whole-database archive in Cloudflare R2 once a day (M5-06). The cloud's synced aggregates are explicitly _not_ the third copy; they carry no invoice counters. | M5         | - [x] settled |
...
The brief from 30 August was explicit: _a prototype in html format then i will see it and make the
call_. So nothing has been built into `apps/console`. What exists is a single self-contained page
covering all four screens, published as an artifact.

**The thesis: an owner does not open this to read a number.** They open it to find out whether the
shop is alright. So the first thing on the page is a state line — _Open · Nimal on Till 1 since 9:04
· last sale 6 minutes ago_ — which is only possible now that M6-13 put a person on the shift, and
which no dashboard template has. The takings follow, and they are priced: _Rs 12,400 ahead of a
normal Monday by this hour_, computable client-side from the trend the console already fetches. A
figure alone answers nothing; a figure against the shop's own rhythm answers the question that was
asked.

**The signature is the pulse.** Today's trade and a normal Monday drawn as two profiles facing each
other across the trading day, with a line marking how far the day has got. An owner on a phone does
not read axes; they read density, and two mirrored shapes say "busier this morning, quieter now"
before any number is read.

**It was rebuilt once, for a reason worth keeping.** The first version drew one tick per sale, which
was truer and unreadable: a hundred and fifty sales across a phone-width morning is a tick every 1.3
pixels, and the graphic collapsed into a solid slab. Binning to the quarter hour keeps what somebody
actually reads off it at any width. Both rows are drawn on **one scale** — two scales would make a
quiet day look like a busy one, which is the only thing the graphic exists to tell apart. And the
sale count in the sentence is written from the same array the graphic draws, because two numbers for
one fact is how a dashboard starts lying quietly.

**Type carries it.** Fraunces for the money figure — a ledger echo, and a serif on a dashboard is the
risk taken deliberately; Archivo for the interface; IBM Plex Mono for invoice numbers and SKUs. Not
Inter, and not the cream-and-terracotta or near-black-and-acid palettes that everything generated
this year arrives in.

**The ground is a warm neutral with a green cast** — the inside of a tea packet — chosen so the
StoreX blue reads as identity against it rather than as chrome. `D3` is unchanged and still enforced:
the logo blue is never behind white text, `--action` at 4.9:1 does every job a person clicks, and
semantic colour never carries meaning without a word beside it.

**Verified in the browser**, light and dark, at desktop width: the tabs switch, the variance opens
with the denominations behind the count, the theme toggle beats the OS setting in both directions.
**Not** verified: the phone layout, because the preview pane renders these pages at its own width
regardless of emulation. The CSS below 900px is a single column with a bottom tab bar and follows
the console's existing patterns, but nobody has seen it on a phone.

#### Built in, 3 September

The call was to build it, so the prototype stopped being a separate artefact. Four things changed
shape on the way from one to the other, and each was the prototype being wrong rather than the code
being awkward.

**The pace sentence was in money and the data was not.** "Rs 12,400 ahead of a normal Monday by this
hour" needs an intraday money baseline, and `PulseSlot` carried only a count. The alternative on
offer was to scale a daily total by the time of day, which invents a shape nobody measured — so the
record gained `usualTotalMinor`, averaged the same way over the same four weekdays. The rule the
prototype set for itself held: every number in that sentence comes from the array the graphic draws.

**The comparison is rounded to the rupee and the takings are not.** An average of four Mondays does
not have cents, and "Rs 26,941.33 ahead" spends the reader's attention on the half that means
nothing. The takings themselves keep every cent, because the prototype's rounded headline would
have disagreed with the sum of the operator rows beneath it — §A's failure arrived at through the
display layer instead of through the maths.

**The muted ink failed AA.** The prototype's `--ink-3` measured 3.3:1 on its own warm ground, and it
sets every eyebrow, hint and timestamp on the screen. Same hue, dragged to 4.9:1. "Muted" was not a
WCAG exemption on the old ground either.

**The ground is scoped to the console.** These tokens live in `apps/console/src/app/globals.css`,
not in `packages/ui` — the same file feeds the till, and repainting a cashier's screen as a side
effect of redesigning an owner's is a change nobody would connect to its cause a month later. The
selectors are copied from `tokens.css` exactly, because a plainer one loses to it on specificity and
the dark console would have kept the old blue-grey silently. `--lum-accent`, `--lum-danger`,
`--lum-pending` and `--lum-ok` are untouched: D3 has measured numbers behind it.

**Kept from the old console, deliberately.** The sidebar-at-`lg` navigation, the login, the theme
toggle and the read-only rule are all unchanged — the redesign was an argument about what a screen
says, not about how it is navigated, and rewriting `AppShell` would have put the tested part of the
console at risk for nothing. The Branches card now appears only when there is more than one branch:
on a v1 shop it repeated the figure above it under a different heading.

**Verified in the running window** at 375px, 800px and 1280px, light and dark, against a fixture
harness that was deleted afterwards: no horizontal overflow at any width, both faces resolving to
the self-hosted files rather than to a fallback, the state line and the pulse reading correctly in
both themes. The phone layout the prototype could not check is checked. 26 tests — 18 on the pulse
arithmetic, 7 on the state line, 1 more on the cloud query — and the page-colour test now reads
`globals.css` instead of holding a second copy of the answer, which is what let the old one pass
while the ground moved underneath it.

### M6-13 — the shift knew who was on the till, and the cloud never did

`V107` gave a shift `opened_by` and `closed_by` in M2, and the outbox dropped both. So the console
could say a drawer was Rs. 250 short and could not say whose drawer, and the question this milestone
was extended for — _who was in the till and how much did they sell_ — had no answer up there at all.

**Both the uuid and the name, which looks like duplication.** The uuid joins to the synced user and
survives a rename; the name is a snapshot for the window where the user has not arrived yet. Shifts
and users are separate aggregates draining through one outbox with **no ordering between them**, so
a shift can land minutes before the person who opened it. A console showing a blank where a
cashier's name belongs, until some later batch happened to drain, looks broken rather than pending.
The query prefers the join and falls back to the snapshot, and both directions are tested.

**Nullable, and permanently so.** Every shift already in the cloud arrived without this and never
will get it: a closed shift is not redelivered, because a synced aggregate is sent when it changes
and that one never changes again. So there is no backfill, the column stays null, and the console
renders **"not recorded"** rather than hiding the row. Hiding it would have been tidier and would
have made this card's total disagree with the day's takings on the card above it — a worse lie than
a blank.

**Attribution goes through the shift, never through the sale.** A sale carries no operator and
deliberately never will: the person is a property of the shift, which is what M2 made the unit of
accountability, and stamping a cashier onto every sale line would be a second place for the same
fact to be wrong. Get this join wrong and the numbers are still plausible, still add up to the day's
total, and belong to the wrong person — which is why it has a test of its own.

**Grouped by person, not by shift.** "Nimal took Rs. 84,000" is the answer somebody wants; two rows
because Nimal took a lunch break is arithmetic homework. The shift count is the visible trace of a
handover for anybody who cares. And the **variance rides along, summed** — takings and drawer
accuracy are the same conversation with a member of staff, and two shifts each Rs. 100 short is a
pattern where one Rs. 200 short is an incident.

**One subtlety in the upsert.** A shift is the only aggregate delivered twice, and the second
delivery is the close. The operator columns are merged with `COALESCE` rather than overwritten, so a
till that has not been updated — whose payload carries neither — cannot blank what a newer one
already told us.

**514 backend tests**, 9 of them new. What is **not** proven: no shift has been closed on a real till
and watched to arrive in the cloud with a name on it. The payload and the ingest are asserted
separately; the round trip is not.

### M6-12 — the half of the console that was designed and never built

M4-07 said the attention feed was cash variance **and** stock variance. Only the cash half shipped,
and `ConsoleReportService` has carried a comment admitting it ever since: _"the stock half joins it
when the console has a stock screen to link to"_. Until now an owner away from the shop could see
the money and not the goods, which is half a business.

**Two questions, and only one of them makes somebody put the phone down.** "What am I about to run
out of" ends with a call to a supplier, so it is first. "What is on the shelf" is a lookup — you
arrive at it already knowing what you want to check — so it is a search box rather than a list to
scroll. Building the second as a browsable catalogue would have been the obvious thing and the
wrong one: a grocery has hundreds of lines and nobody scrolls them on a phone.

**Σ movements, on both tiers, forever.** §A's second rule pays off here for free: the cloud receives
movements, so on hand is a sum and two branches reconcile by addition. The temptation is a cached
level — this is a read-heavy screen over a table that only grows — and the cost of it would not be a
slow page but a number that disagrees with the till's. An owner and a shopkeeper reading different
figures off two screens, with no way to tell which is wrong, is worse than a slow screen by a wide
margin.

**Blank and zero stayed different, which took one line of care and one test.** `rs.wasNull()` rather
than a zero check, because `getInt` returns 0 for SQL NULL and reading that as "not watched" is
precisely the bug V120's comment was written to prevent. Two tests hold the line: an unwatched
product is never listed however empty, and a threshold of 0 fires exactly at empty.

**A negative gets its own word.** On hand below zero means a sale was rung up for stock the shop
never recorded receiving. Calling that "low" would send somebody to a supplier when the fix is in
the goods-in book, so the screen says **not received** and the test asserts the figure is reported
rather than clamped.

**One test that guards a schema change nobody has made yet.** The movements subquery is filtered by
tenant as well as the outer query. Two shops cannot share a product uuid today; they are one schema
change away from it, and the failure would be two shops pooling their stock silently.

**505 backend tests**, 11 of them new. The screen loads its own data rather than joining the page's
opening fan-out, so signing in to look at today's takings does not pay for two catalogue scans.

### M6-11 — reaching the thirty-first shift

The history returned the newest thirty and there was no way to ask for anything older. A shop trades
for years. That is not a list, it is a window with no handle.

**Not pagination, and the reason is what a shopkeeper says out loud.** They ask "show me last week"
and they ask "show me more" — a question narrower, and an answer longer. Those are two controls, and
page numbers are a third concept that answers neither. On a list that only ever grows at one end,
page three is a different set of shifts tomorrow.

**`OFFSET` is wrong here, not slow, and the failure is quiet.** A shift closing on the other till
while somebody is reading pushes every row down one, so the second page starts one row too early and
repeats the shift at the boundary. Nothing is lost — which is exactly why it is dangerous. A
duplicate in a list being reconciled against a drawer reads as two events rather than one, and the
arithmetic that follows is wrong with no visible cause. Comparing on `(closed_at, id)` asks for what
is genuinely after a specific row and cannot repeat or skip. Both halves of the tuple matter: two
tills can close within the same microsecond.

**A correction worth recording.** The first version of this note, and of the test that went with it,
claimed `OFFSET` would _skip_ a shift. It does not — growth at the top produces a duplicate; a
_deletion_ produces a skip, and shifts are never deleted here. The test now asserts the thing that
actually happens, which is the only version of it worth having.

**Half a cursor is refused rather than honoured.** A `beforeId` with no timestamp would compare
against NULL and return an empty page — indistinguishable, on screen, from "there are no more
shifts". Both or neither.

**Where the tests had to trade.** `ShopFixture.seed()` gives each test a new _branch_ under the one
tenant a desktop database may hold, and this report is shop-wide by design — so every other test
class's shifts are in the results. Three assertions failed on that before the suite was moved to a
window two hundred days back that nothing else uses. The alternative, filtering the report by
branch, would have been changing the product to suit the test.

**494 backend tests**, 7 of them new. The date inputs and the button are not covered by the e2e
suite, which does not currently open the shifts tab at all.

### M6-10 — the alert that could not be cleared, and the console's first write

The sharpest observation in the 30 August list was one sentence: _"attention tab have 11 attention
cant do anything about it"_. It was right, and it was a design bug rather than a missing feature.
The feed counted eleven variances and offered no way to open one, no way to say "I checked, it was
my mistake", and no way to make it go away. It would have counted twelve, then twenty, and an owner
would have stopped opening the tab — which kills the one screen the console exists for. **An alert
nobody can clear is not an alert. It is wallpaper.**

**Reviewed is not gone, and that is half the feature.** There is a second list. Every cleared
variance is in it with the name of whoever cleared it and when. Without that half, the button is a
way for anybody with the owner's phone to make a shortfall disappear from the only screen that would
have shown it — which is the opposite of what this screen is for.

**A table, not a boolean, and the reason is the sync path.** The obvious implementation is
`shifts.acknowledged`. It would have worked in testing and failed in a shop: the shift is a **synced
aggregate**, delivered by the till and upserted here, and it is the one aggregate delivered
**twice** — open, then close. The next delivery would overwrite the flag and a reviewed variance
would silently un-review itself. Beyond that it is a different fact with a different author: the
shift is what the till says happened, and this is what somebody in the office did about it, days
later, from a phone. The same instinct as the stocktake's `ABANDONED` and the shift's own
`closed_by` — somebody acting is new information, not the absence of old information.

**The console can now write, and it took a paragraph to be comfortable with.**
`AuthenticatedPrincipal` has said since M4-05 that a console session reads and never writes, and
that a stolen one discloses takings and cannot alter a figure. Both halves of that mattered. What
made the widening acceptable is how narrow it is: no money, no ledger, cannot change what a shift
says, attributed to the session that did it, and the shift stays listed under `?reviewed=true`
afterwards. **A stolen session can hide an alert; it still cannot hide a shift.** The alternative was
making an owner walk to the till to clear a variance they are looking at on their phone, which is
the whole reason the console exists. The class doc, the API client header and the console page's own
comment all say so now — three places that claimed "never writes" and would otherwise have been
quietly wrong.

**The detail is the denominations, and they were already there.** "Rs. 5,000 short" is the alarm;
whether it was one note or a hundred coins is the next question and it decides which conversation
the owner has. `shift_counts` has carried that since V203 with a comment saying exactly why, and
nothing had ever read it.

**Two small things worth remembering.** The acknowledgement row names a **person** and the request
carries a **session**, so the console user is resolved from `console_sessions` inside the service
rather than by widening `credentialId`, which means "the credential presented" for all three
credential kinds and would stop meaning one thing. And Maven's incremental compiler happily
reported BUILD SUCCESS against test sources it had not recompiled after the service signature
changed — `clean test-compile` is what actually answers that question.

**487 backend tests**, 10 of them new. What is **not** proven: nobody has pressed the button on a
real phone against the deployed console.

### M5-11 — the update channel, and the button that does not exist

Two decisions, and both are refusals.

**It never installs while the app is running.** `autoInstallOnAppQuit` is on, `quitAndInstall()` is
never called, and there is no "Restart now" — not in the notice, not in the preload bridge, not
anywhere a renderer could reach. This is not "we try not to interrupt a sale". There is no code path
that can. Every desktop app ships that button; on a till it is a way to stop Postgres under an open
shift, lose an unprinted receipt and leave a cashier holding a queue. The update is applied by NSIS
after the process exits, which for a grocery is the end of the day.

The cashier gets one line — _"Update ready. It will be installed the next time StoreX is closed."_ —
and nothing to press. It exists because the alternative is a POS that silently changes overnight,
and a shopkeeper who opens on Tuesday to a screen that moved deserves to have read on Monday that it
was coming.

**It refuses to run unsigned, and that is why the task is `[~]` and not `[x]`.** An update channel
is by construction remote code execution on every shop PC that has the product. The only thing
standing between it and whoever can answer for that URL is the Authenticode check
`electron-updater` performs on the downloaded installer — and that check happens only when the app
was signed with a publisher name to compare against. An unsigned channel is not "an update channel
with a SmartScreen warning"; it is an open door. So `updateDecision` refuses without one, refuses a
plaintext feed, refuses an unpackaged build, and says which in a log line somebody will find. The
override for testing the mechanics is named `LUMORA_UPDATE_ALLOW_UNSIGNED` and shouts
`ENABLED WITHOUT SIGNATURE VERIFICATION` on every launch.

**What that decision cost, and why it is still the right one.** The feature ships switched off and
stays off until somebody buys an OV certificate — $200–400 a year and a week or two of proving the
company exists. The alternative was a channel that works today and that anybody able to spoof one
hostname could use to own every till in the estate. `docs/releasing.md` carries the procedure, the
certificate requirements and the `publisherName` trap: it must match the certificate's subject
exactly, because that string is what every till compares against and a mismatch means the whole
estate silently stops updating.

**The build config names no domain.** The publish URL is `${env.LUMORA_UPDATE_URL}`, interpolated at
pack time, because there is no update host yet and a placeholder domain written into a config file
is how a placeholder becomes a production value nobody re-read.

**The one fiddly bit.** `electron-updater` had to be added to the asar, and M5-01's `files` list is
an explicit allowlist — that is what keeps the installer at 178 MB rather than packing
`node_modules` wholesale. Its runtime closure is fourteen packages; they were computed from
`node_modules` rather than guessed, and listed.

**10 tests**, all on `updateDecision` and the publisher-name read, which are the parts that are ours.
The downloading and the swap belong to `electron-updater` and are not tested here. **102 terminal
unit tests** in total. What is **not** proven: the channel has never run, because there is nothing to
sign the build with.

### M5-10 — erasure, and the two opposite ways to get it catastrophically wrong

Destroy too little and the shop is liable for up to Rs. 10M an instance. Destroy too much and its
own accounts stop reconciling — a Z-report printed last week becomes a lie, and the shopkeeper finds
out during an audit. Almost every test in `CustomerPrivacyTest` is about the second one: it rings up
real sales, erases the customer, and then asserts the money has not moved.

**A delete was never available, and the code already said so.** `CustomerService.setActive` carried
a note from M3-11 — sales reference this row, a delete would fail against the foreign key or be made
to succeed by loosening it, and "erasure under PDPA is a deliberate act that has to decide what
happens to the invoices". That decision is now made: the personal fields are overwritten, the row
keeps its id and its `client_uuid`, and every sale still points at the same party, which is now
nobody in particular. That is anonymisation, and it is what the law asks for where erasure meets a
retention obligation rather than a way around it.

**The export carries line items rather than a total.** "You spent Rs. 4,200 across nine visits" is a
summary of the data; the right is to the data, and what the shop actually holds is that this person
bought these things on these days. It downloads as a file rather than filling a screen — somebody
exercising this right needs something they can keep and check, and a shopkeeper reading figures off
a monitor has not answered the request. The file is named by customer id, because a downloads folder
full of files named after people is itself a pile of personal data.

**An erasure that stops at the shop PC is not an erasure.** The cloud holds the name and the number.
The customer aggregate has always been sent whole rather than as a diff, so re-enqueueing the
blanked row overwrites what is up there through exactly the path that put it there — and the outbox
row is written **in the same transaction** as the erasure. A shop cannot end up having told somebody
yes while the cloud still holds their number. `V213` adds `erased_at` on the cloud side too, so the
console shows a state rather than a placeholder name that reads like a bug.

**What it refuses to erase, and why that is defensible out loud.** An issued tax invoice snapshots
the purchaser's name, TIN and address because Gazette 2481/22 requires the document to show them,
and the purchaser filed it and claimed input credit against it. A shop does not get to unilaterally
revoke a statutory document somebody else is relying on. The erasure result reports the count, which
for a grocery customer is essentially always **zero** — a tax invoice is issued on request to a
VAT-registered purchaser, not to somebody buying a loaf.

**One design reversal, made because of a test.** The first version filtered erased rows out of every
search, including the back office's. That is wrong twice over: the row has nothing personal left on
it, and hiding it leaves a shopkeeper unable to answer "did you erase my details" — the question that
follows the request. It now relies on the chain that already existed: erasure deactivates, and
`setActive` refuses to undo an erasure, so the active-only search the till uses can never return one
while the back office sees it with inactive rows included. Fewer clauses, and the invariant is
enforced where it belongs.

**One real bug the tests caught.** The name branch of `search` uses `lower(c.name) LIKE ?` and not
`ILIKE`, so a first pass at the filter missed it entirely — searching "Erased" would have listed
every erased person in the shop. It failed on the first run, which is the only reason it is not
still there.

**477 backend tests**, 11 of them new here. What is **not** proven: nobody has run an erasure against
the installed till and watched the blanked row reach the cloud. The transaction and the payload are
asserted; the round trip is not.

### M5-06 — the copy that is somewhere else, and the store that would have evaporated

`M5-04` answers the accident: a bad migration, a deleted table, an index that rotted. It never
answered the disk. `LUMORA_BACKUP_DIR` can be pointed at a USB stick and mostly is not, and the
default folder sits beside the data on the same disk that died. §A puts a shop's entire trading
history on one machine under one counter, and this is the price of that.

**The eleven synced aggregates were never a second copy, and saying so in `restore.md` was not
enough.** They carry no `invoice_counters`, so a shop rebuilt from them re-issues numbers already
printed on paper in a customer's hand. What goes up now is the till's own `pg_dump` output — the
whole database, counters included, restorable with the same commands as a local backup. The
document now says the cloud holds **two different things** and only one of them is a shop.

**Once a day, not twice.** The local backup runs every twelve hours because it costs nothing. This
one crosses a connection that in Sri Lanka is often mobile and metered, and a second copy of the
same day's trading is worth very little. Twenty hours rather than twenty-four, so a shop that opens
at eight today and nine tomorrow still gets one every day instead of skipping every third.

**The decision that had to be asked rather than assumed.** The obvious implementation writes the
archive to the cloud host's filesystem, and on Render's free tier that container is recreated on
every deploy. Uploads would succeed, rows would be written, the till would be told everything was
fine, and every archive would be gone at the next release — the worst failure mode this system can
have, because it is indistinguishable from working. Neon was no better: 0.5 GB holds every shop's
ledger and a single shop's dumps would evict it. Cloudflare R2 was chosen on 2026-09-01: 10 GB free,
**no egress charge** — which matters precisely because the day anybody reads one of these is the day
a shop downloads its whole database — and the S3 API, so this is not soldered to a vendor. The
filesystem store still exists and is what the test suite uses; `CloudBackupConfiguration` logs
which one is live at startup, and warns by name when it is the ephemeral one.

**The AWS SDK is the only third-party SDK in this service, and it costs the installer ~10 MB.**
One jar ships to both tiers, so a cloud-only dependency lands on every shop PC. Hand-rolling SigV4
would have saved it and put a signing bug between a shop and the only off-site copy of its history.
That is the wrong thing to economise on.

**Nothing here trusts the till.** The archive's name arrives in a header and becomes part of an
object key, so it is matched against the till's own naming — `storex-YYYY-MM-DDTHHMM.dump` — and
nothing else; five hostile names are asserted, including a percent-encoded traversal. The key is
built from the **authenticated** tenant, never from anything sent. And the digest is recomputed
from the bytes that actually arrived: a mismatch deletes the object rather than leaving an archive
of unknown contents filed beside ones whose contents are known.

**Two orderings, one rule.** The object is written before the row; retention deletes the row before
the object. Both say: never a row without its object. An orphaned object costs a fraction of a cent
and is invisible. A row pointing at nothing is a lie told to somebody standing in a shop at 8am with
a dead PC, and they believe it until the download fails.

**One thing the tests caught that review would not have.** The first version of the Electron suite
redirected `LOCALAPPDATA` inside each test and called `vi.resetModules()`. That does not reach a
`require`d CommonJS module's captured constants — so the tests wrote their state file into the
**real installed till's config folder**, where it would have silently suppressed that shop's next
cloud upload. It did not fail; it passed the first test and then reported `not-yet` for every one
after. The environment is now redirected at file scope, before anything can capture it.

**And one piece of duplication that was a bug waiting.** `backend.cjs` resolved the cloud
credential inline — wizard first, then the machine environment, the order that exists because a
stale `setx /M` token once filed an afternoon of sales under a previous shop. `backup.cjs` needed
the same answer. Two copies of that rule would eventually disagree, and the disagreement would put
one shop's entire database in another shop's storage. It is now one function, `cloudCredential()`,
and both callers use it.

**466 backend tests**, 11 of them new here, and 92 terminal unit tests including five that stand up
a real HTTP server and upload a real file to it. What is **not** proven: no archive has yet been
uploaded to an actual R2 bucket — the bucket, the token and the five environment variables are a
deployment step nobody has performed. `DEPLOYMENT.md` carries the procedure.

### M5-01, M5-02 — the till becomes something you can install

**What ships.** One 178 MB `StoreX-Setup-0.0.0.exe`: Electron, a 44 MB Java runtime, Postgres 16.6 pruned to 102 MB, the backend jar and the Next standalone build. Installed and run on this machine — full stack up in **20 s** on a first launch including `initdb`, **11 s** on the second, clean shutdown in **2 s** with no orphaned processes and no stale `postmaster.pid`.

**The JRE is built, not downloaded.** `jlink` against the JDK already required to build the backend produces 44 MB where a full Temurin JRE is ~180 MB, and removes a download from the build. The module list is explicit and slightly generous because **`jdeps` is useless here**: run against a Spring Boot fat jar it reports `java.base,java.sql`, having seen only the launcher and none of the nested libraries. Trusting it would produce a JRE that boots and dies on the first JDBC call. The list was verified the only way that means anything — by starting the real jar on the result and watching `/actuator/health` come UP.

**Three things only running it could have found**, all in the launcher, none visible to any test:

1. **`pg_ctl start -w` never returns under Node.** It launches the server as a child inheriting its stdio, then exits — but `execFile` resolves on _stream close_, not process exit, and the still-running server holds those pipes for its whole life. The callback was effectively scheduled for whenever the shop closes forever. Fixed by spawning `postgres.exe` detached and polling `pg_isready`.
2. **A free-port check is the wrong question.** Docker publishes on `0.0.0.0`; our server binds `127.0.0.1`. Both "succeed" on 5442, and a client connecting to `127.0.0.1` reaches whichever Windows routes it to. It reached the **container**, the backend reported healthy, and the bundled cluster sat there with no database in it — a till that appears to work while writing every sale into a database the installer does not own and no backup covers. The guard is now `SHOW data_directory`: not "is this port free" but "is the server on it the one whose data directory I control".
3. **`spawnSync` freezes the splash.** Electron's main process is single-threaded and also draws the window, so a synchronous `initdb` left the app Not Responding through the slowest part of the first launch. Everything in `postgres.cjs` is async now except `stop()`, where there is no UI left to block.

**Also worth keeping.** The asar packed **211 MB of node_modules** — Next, React and the SWC binaries, all of which belong to the _renderer_ and already exist in `runtime/web`. The main process needs exactly one runtime dependency, `serialport`. Excluding the rest took the archive from 73 MB to **1 MB**. And the Next rewrite destination is **frozen into the build**: it appears in both `server.js` and `routes-manifest.json`, so no environment variable can move the backend's port at runtime. 8081 is therefore a hard constant the launcher refuses to work around, which matches what CLAUDE.md already called it.

**A fourth failure found by looking at it, not by testing it.** The first person to run the installed till was shown **a stack of black console windows** across the screen, multiplying as it started. `detached: true` on the postgres spawn — added to say "this outlives the call" — implies `CREATE_NEW_CONSOLE` on Windows, so the child gets its own console window, and `windowsHide` does not override an explicitly requested console. Postgres then forks a checkpointer, a walwriter, an autovacuum launcher and one worker per connection, each inheriting the same treatment. Dropping `detached` fixes it with nothing lost: `stdio: 'ignore'` means there are no pipes holding the call open and `unref()` releases the child from the event loop, so the server still outlives the function. Now measured rather than assumed — **0 visible console windows**, 45 postgres processes, none with a window, one StoreX window.

**A trap that cost the most time, and was mine.** `ELECTRON_RUN_AS_NODE=1`, set for one probe early on, persisted in the shell and was inherited by every launch afterwards. It makes any Electron binary run as plain Node, so the app exited instantly with no window, no log and exit code 0 — and every "fix" appeared to fail. If a packaged Electron app exits silently with status 0, check that variable before believing anything else.

**Still open.** The NSIS Defender-exclusion hook could not be verified — reading `Get-MpPreference` needs administrator rights this session did not have. It is written and runs at install time; nobody has confirmed it took effect. And the shop is **empty on first run** — no tenant, no users — so the login screen cannot be used yet. That is `M5-03`'s first-run wizard, and it is now the thing standing between this installer and a pilot shop.

### M3-15 — low stock, and the difference between blank and zero

**The one report about tomorrow.** Every other report answers a question about the past. This one is the only one a shopkeeper can still act on, which is why it sits as a Reports tab beside top products rather than under Stock: "what sells" and "what am I about to run out of" are one decision read in one sitting.

**Nullable with no default, and that is the whole design.** `products.reorder_point` is `integer NULL`. `DEFAULT 0` would have been the smaller migration and is wrong, because **0 is a real threshold** — a shopkeeper saying "tell me the moment this is empty". If 0 also meant "unset", the one product they cared most about would be indistinguishable from the four hundred they never configured. That distinction then has to survive three more crossings, and each was a place it could have been quietly lost: the ingest uses `optionalLongOrNull` rather than `optionalNumber` (which returns 0 for an absent field); the form keeps the draft as a **string** so `''` and `'0'` stay apart; and both were verified against a running cloud, not just asserted — a 0 set at the till arrives in `lumora_cloud` as 0, not null.

**An unwatched product is never listed, however low it goes.** This reads like a gap and is the feature. A list nobody curated fills with things legitimately near zero — the slow line, the one-off, the item ordered in for a customer — and a list that is mostly noise is one a shopkeeper stops opening. Which is the same failure the console's attention feed already has, and worth fixing there for the same reason.

**A bug the comment found before the test did.** `ProductAdminService.save` writes the whole row, so passing `null` from the import path would have **silently cleared every reorder point somebody had set by hand** — on a screen whose entire promise is that it only changes what is in the file. It was noticed while writing the comment explaining why an import carries no threshold. `ProductImportService` now reads the existing values and passes them back, and two specs hold it: an import does not clear a hand-set point, and an imported product arrives unwatched.

**Two things only running it could have found.** The frontend called `/api/stock-on-hand/low` when `StockController` is mapped under `/api/back-office` — the tab rendered perfectly and the fetch 404'd, which no unit test would ever have seen. And the back office's sidebar `<nav>` had no accessible name, so `getByRole('button', { name: 'Products' })` matched both it and the Reports screen's "Top products" tab; the sidebar is now `aria-label="Back office"`, which is a genuine screen-reader improvement and not only a test fix.

**A trap for the next session.** `LUMORA_CLOUD_URL` in this shell pointed at the **deployed Render cloud**, not local `:8082`, so early sync checks reported "accepted" while nothing appeared in `lumora_cloud` — the rows had gone to production, which does not have `V211`. Check that variable before concluding sync is broken. **`V211` is not applied to the deployed cloud**, so a till syncing there will have its `reorderPoint` dropped until it is.

**427 backend tests** (was 415), **53 e2e specs**, 170 domain, 84 terminal, 15 console.

### The arrow keys skipped a line, and only in dev

Reported from the shop floor of a `pnpm dev` session: arrow keys moved the cart selection **two** lines per press, so the middle of three items was unreachable.

`useCart.move` called `setSelected` with an **updater function** from inside a `setLines` updater. React requires updaters to be pure and invokes them **twice under StrictMode** precisely to expose impurity — so the nested `setSelected(s => …)` ran twice and applied its delta twice. The four sibling calls in `addProduct`, `changeQty` and `voidLine` have the same shape and are harmless only because each passes a _computed constant_: running twice sets the same value twice. `move` was the one that passed a function. Fixed by reading `lines` from the closure instead of nesting, and the rule is now written on the function.

**The part worth remembering is how nearly it was mis-fixed.** The e2e suite runs a **production build** (`playwright.config.ts`, so keystroke-timing specs are deterministic), and StrictMode double-invocation does not happen there. The regression test written for this bug was run against the _reverted, broken_ code as a check — and **passed**. Without that check the fix would have shipped with a test that proved nothing, and the next person would have trusted it. The spec is kept for the non-StrictMode case with a header saying plainly what it does not cover; the actual guard is the rule on `move`.

Two smaller notes from the same session. `dev-seed.sql` gives **Milk Powder 400g two barcodes** (`4791234567937` and `8901234567895`) — a test that scans both gets one merged line, not two, which reads as a broken cart. And `next dev` cannot share `apps/terminal/.next` with the suite's `next build`: run them together and the dev server 404s its own chunks.

### M6 — the till learns to be touched, and the keyboard loses nothing

**The shape.** Two columns: the receipt takes everything the 300px action rail does not, with the totals **beneath** the cart rather than beside it — where every commercial till puts them, and where the eye lands after reading down the lines. The rail is a quantity keypad over Search / Void / Clear / PAY. The F-key bar keeps all twelve slots in their original order and grew from 56px to **84px**, which is the change that makes it a control a finger can hit rather than a legend that happens to be clickable.

**No product grid, and that decision paid for itself twice.** The catalogue does not fit on a screen, the gun beats a grid for anything with a barcode, and F3 covers the rest. Dropping it removed the entire backend half of the milestone — `ProductSummary`, `ProductLookup`, `ProductController` untouched, no endpoint, no migration — and with it the worst failure mode on the list: a shared SQL column list where an `INNER JOIN` instead of a `LEFT` would have hidden every uncategorised product from the grid _and from search_, which is a brand-new shop opening an empty till.

**The one rule.** No touch control introduces a new code path; every button calls the callback its function key already calls. That is why the keyboard specs needed no rewriting, and why each rail button carries its F-key — a cashier who taps `Void line` sees `F4` and learns the shortcut for free.

**`handEvents()` — a predicate weakened once, on purpose.** `keyboard-only-sale.spec.ts` had carried the instruction since M1-16: _"If that ever changes deliberately, the assertion to keep is `detail === 0`."_ It has now fired. A keyboard-activated `<button>` fires a synthetic click with `detail: 0` and no `pointerdown`; anything a hand did produces one or the other. Both halves of the filter are needed — `detail > 0` alone misses a tap that slid off its target. Landed as its own commit with no app code, so "the suite passes identically" was provable rather than asserted, and the detector spec now proves the filter in **both** directions: it keeps a real click and drops a synthetic one.

**Digits stay unbound, and must.** The keypad is touch-only. `scanner.ts` never binds a plain digit because a barcode is a burst of digits, so binding `0-9` would make every scan type a quantity. The tempting shortcut — dispatch synthetic `KeyboardEvent`s from the buttons to reuse the existing handlers — would advance `lastCharacterAt` and break the M1-09 terminator window under a real gun, a failure that only appears in a shop. There is a spec that taps five keys and then checks a human-paced Enter still searches.

**Three things the tests could not have caught, found by looking at the screen.**

1. **PAY was drawn behind the F-key bar at 1024×768.** The rail's natural height won over its flex parent, so the tallest thing on it simply went below the fold. `min-h-0` on the column and `flex-1` on the keypad grid make the _keys_ give up height instead — the actions at the bottom are always whole. Screenshots at both widths, not a viewport assertion, are what surfaced this.
2. **`--lum-ok` needed an ink companion, and the reason is the same one the brand/accent split exists for.** The readable text colour on filled green **inverts between the themes**: the dark theme's `#3ec93e` measures 8.5:1 against near-black and 2.2:1 against white, while the light theme's `#0a7d0a` is 5.3:1 against white and 3.5:1 against near-black. A hardcoded colour on the PAY key would have failed AA in exactly one theme, silently, because nothing tests a colour. `--lum-ok-ink` is now defined in all three palette blocks.
3. **Two controls whose accessible names both began "Clear."** The keypad's backspace was "Clear quantity" and the rail's was "Clear" — ambiguous to a screen reader for precisely the reason it was ambiguous to Playwright. Renamed "Undo quantity".

**The green PAY key is a deliberate exception to §A** ("accent is the primary action and nothing else"), recorded beside `--lum-ok`. A green pay key is a forty-year convention on this hardware; breaking it for token purity would cost more than it buys, and the accent stays free for selection state.

**A pre-existing flake, fixed on the way past.** The suite had been intermittently failing with a dropped leading barcode digit — "No product for barcode 791234567890" — which reads as a missing product rather than a timing fault. The fixture waited for `#scan` to be _visible_, but the field takes focus in a `useEffect` that runs after paint, so a spec could start typing into nothing. It now waits for **focus**. Unrelated to M6, and it had been costing runs for a while.

**`packages/ui` stays empty, deliberately.** Its own comment says components arrive with the surfaces that need them, and exactly one surface needs these. The console is a phone PWA with the opposite constraints, so a shared button would immediately need `variant="till" | "console"` — two components in a trench coat. Only tokens were added.

**Reruns and stale servers.** `playwright.config.ts` sets `reuseExistingServer: true` for the Next server, which serves a **production build**. After changing a component, a leftover server from an earlier run will happily serve the old bundle and the failure looks like a missing element. Kill whatever holds :3000 before concluding the code is wrong — that cost a diagnosis here.

**51 e2e specs** (was 38), 84 terminal unit, 170 domain. **Gate M6 is outstanding** and needs a person: one full sale with no keyboard, one with no screen, on the same till.

Append-only. One row per gate attempt (pass or fail) and per significant course change.

| Date       | Milestone   | Gate result | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ----------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-01 | M5-03       | —           | **The wizard shipped with the exact failure it was built to remove, and a real onboarding found it.** A till was set up for a new shop, the token was pasted, the wizard said "Saved." — and nothing ever synced. Two faults, both mine. **The cloud address field was labelled optional** and hinted "leave blank unless you were given one", so it was left blank; with nothing set, `application-desktop.yml` falls back to `http://127.0.0.1:8082`, which is the till itself. **And the token was never checked** — the wizard verified only that the field was non-empty, and non-empty is not a credential. The cloud returns 401 for it. Discovered by reading `runtime.json` and counting outbox rows (62 queued, 0 acked), which is not something a shopkeeper does. That is precisely the silent failure this wizard replaced `setx /M` to prevent, restated in its own words in the file header, and it survived into the replacement because the check that mattered was never written. **A field being present is not the same as a field being right, and "optional" on a field the system cannot default is a lie.** Fixed: the two cloud fields are now a pair — both or neither, refused with a reason naming the consequence rather than the rule — and `CloudCredentialCheck` asks the cloud before the shop is created. Ordering is deliberate: a rejected token is fixable while the person is still on the screen with the platform page open, and unfixable once they have finished. It probes the **entitlement feed** specifically, because that is the one endpoint `TenantAuthFilter` allows a lapsed licence to reach — a shop whose licence ran out yesterday still has a valid token and must not be blocked from setup. The distinction the whole class turns on is **"the cloud says no" vs "the cloud did not answer"**: only the first blocks. §A says the network is on the critical path of nothing, and a shop being set up in a back room with no signal is being set up correctly — an unreachable cloud saves the token and says so. An address that answers but is not this API reads as unreachable too, since telling somebody their token is bad would send them to reissue a credential that was fine. On success it shows the **shop name the cloud returned**, which is the answer worth reading: a green tick confirms something answered, a name catches a valid token belonging to a different shop. Seven tests against a real `HttpServer` rather than a mocked client — the behaviour under test is entirely about the wire, and a mock would assert that the code branches on values the mock chose. Deserialises into a narrow local record, not `sync.Entitlement`: binding the shared contract would make an older cloud's response fail during setup, turning "your cloud is a version behind" into "your token is bad". **Still unfixed on the installed till:** it runs schema 121 and predates V122, so it has no mismatch warning — the installer needs rebuilding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-09-01 | M5-07 M5-08 | —           | **Crash safety, asserted without killing anything.** "Kill the process between writes" reads like it needs a killed JVM, and a test that did that would prove _less_: it samples one instant chosen by the scheduler, and it is flaky in the way that teaches people to re-run rather than read. What a crash between two writes **is**, to Postgres, is a transaction that never committed — so the failure is injected with a trigger that raises inside the server, where a full disk would fire, and the assertion is on what survives. Reproducible every run, and it tests Postgres's rollback rather than a mock's. `SaleCommitTest` already used this technique for the outbox; this generalises it. Seven tests. The sale half: an interrupted sale leaves **no header without lines** (which would be an invoice number reconciling to nothing), **no stock movement for goods that never left** — the dangerous direction, because stock on hand is the sum of movements, so an orphan permanently changes what the shop believes is on the shelf with no sale to reconcile against — and **no burned invoice number**, since the allocator runs inside the caller's transaction and a rolled-back sale gives the number back. The fourth asserts the till then sells normally, because not-corrupt is only half of recovered. The shift half is M5-08, and its sharpest test is the interrupted **close**: a shift stuck half-closed would be unrecoverable, because `ux_shifts_one_open_per_terminal` means the till refuses to open another one and the next morning fails with an error about a shift nobody can find. It stays OPEN, and closes cleanly on the retry with its sales still attached. One bug in the test itself, worth recording because it is a trap in this codebase generally: `queryForObject` **throws** on an empty result rather than returning null, and "no invoice counter row yet" is a real state — the block is created lazily on first sale. `queryForList` and an explicit empty check. **What this does not claim:** nothing here proves Postgres survives a power cut — that is fsync and WAL, and it is the database's job. What is proven is that this application groups its writes so that guarantee is worth having; six statements on autocommit would lose a sale whatever the WAL did. The real power cut on real hardware is Gate M3's cable-pull and the pilot, and this is what makes attempting them reasonable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-09-01 | M5-04 M5-05 | —           | **Backups, and the restore that is the only reason to have them.** `pg_dump` twice a day rather than nightly — a 3am backup is one a shop that closes at eight and switches the PC off never takes, and twice from launch means an ordinary trading day produces at least one either way. First run is deferred five minutes: a dump competing with Postgres starting, Flyway migrating and the first screen building is felt by the cashier already waiting. Written to `.part` and renamed on success, because a truncated archive that looks good is worse than a missing one — it is the one somebody restores from. Every archive is read back with `pg_restore --list` before it counts as taken, and an unreadable one is **kept**, not deleted: it is the evidence. Pruned by age (30 days) not by count, because "the last 10" means two months on a machine used twice and five days on one left running. **Nothing new ships** — `pg_dump.exe` and `pg_restore.exe` were already inside M5-01's pruned 102 MB Postgres, which is worth knowing before anyone proposes bundling a backup tool. The tests run the **real** `pg_dump.exe` against the real container rather than asserting on arguments: a mocked backup test verifies the arguments and none of the failures that actually lose a shop's history — a password that never reaches the tool, a format `pg_restore` cannot read, a partial file where a good one should be. M5-05's test dumps a table, **drops it**, restores into a database that never had it, and asserts the money adds up (133490 minor) rather than the row count — a restore that brings back two empty rows is a restore that lost the money. `docs/restore.md` carries what a person does, and two things in it were wrong until checked against the source: the outbox column is `acked_at` not `sent_at` (a row clears when the cloud confirms, not when the till posts), and the tools are under **Program Files**, not `LOCALAPPDATA` — `perMachine: true`, which is also what lets an uninstall keep a shop's data. A wrong path in a recovery runbook fails at the one moment it is read. The document's own hardest section is the honest one: **the cloud is not a complete second copy.** The outbox carries eleven aggregates and does _not_ carry branches, settings, or `invoice_counters` — so a shop rebuilt from the cloud alone would re-issue invoice numbers already printed, which for an IRD document is a compliance problem. Hence the counter check after every restore, and hence the local backup being primary. **M5-05 is left `[~]`, not ticked**: the dump→restore cycle is proven by test, the _procedure_ is not — "actually tested" means a person on an installed till, and nobody has. Also cleared today: yesterday's `V121`/`V122` and the wrong-shop fix now have real evidence (**441 backend tests green**) after Docker Desktop had stopped mid-session and turned 439 passes into 439 connection refusals. Three `new Entitlement(...)` call sites in tests needed the new field, and two new tests assert the shop name round-trips and that a cloud sending no name does not erase one already known                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-08-31 | M5-03       | —           | **The first-run wizard, and a NOT NULL that broke ten tests.** Four steps behind `/api/setup`, unauthenticated because on a fresh till there is nobody to authenticate as — **the empty `tenants` table is the credential**, checked inside the transaction that would create the second shop, which is the shape `PlatformBootstrap` already uses for the cloud's first admin. A shop is one transaction: tenant, branch, settings, OWNER user and a block for **both** document types, so a shop's first refund is not the moment its credit-note counter appears (V108). The five hardcoded constants in `page.tsx` are gone; `Page` is now a gate rendering the wizard, the till, or an error — and deliberately **not** the wizard on an error, because offering setup to a shop that already exists is how a till acquires a second identity. **The surprise: `V121` added `terminal_code text NOT NULL` and put 30 errors into a green suite.** `tenant_settings` is not owned by setup — `TenantSettingsService` and the shift path both upsert into it with only the column they care about, creating the row lazily, and a NOT NULL with no default turns every one of those into a constraint violation. The fix is `DEFAULT 'T1'`, which reads like a contradiction of V121's own argument that a default terminal code is the dangerous thing, and is not: the _wizard_ must make somebody choose and does, while the _column_ default exists so an unrelated write to an unrelated column does not fail. **A schema change is not safe merely because production writes through one path.** Second lesson, in the test: emptying the database by DELETEing a hand-ordered list of tables was wrong twice in a row (`stock_movements` → `users`, then `refund_items` → `sale_items`), because that list has to be re-derived every time a migration adds a table. `TRUNCATE tenants CASCADE` says what is meant and stays true as the schema grows; it is transactional in Postgres, which is what lets it roll back and leave the shared fixture alone. The rollback test suspends the class transaction with `NOT_SUPPORTED` — inside one, the assertion would pass against a service with no transaction at all. **The cloud token now lives in `runtime.json`** (0600, beside the database password) rather than `setx /M`: no elevated prompt, no invisible value, and the wizard verifies before saving. It takes effect on the next launch, because `HttpCloudSyncClient` bakes the header into its `RestClient` at construction — the wizard says so on screen rather than pretending otherwise. A machine-level `LUMORA_CLOUD_TOKEN` still wins, so the two pilot tills activated by hand are not silently de-activated by this. **Not yet run against the installer**, which is the one thing that would prove it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-08-29 | M4-10       | —           | **`pnpm check:cloud`** (`scripts/e2e-cloud-check.mjs`) — a trading day rung against the real till and checked against the real cloud: shift open, six sales (plain cash, line discount, mixed/zero-rated VAT, split tender, sale-level discount, cash rounding), a safe drop, a receipt-linked partial refund, replay, and a shift close with a deliberate Rs 100 variance. **41 checks, all passing.** It is not a substitute for `pnpm test` or `./mvnw -B test` — what it covers is the part that only exists once deployed: that the arithmetic survives serialisation to Singapore. Every assertion compares two independently computed numbers, the till's and the cloud's, so a shared bug cannot make it pass. Writing it was more useful than running it, because **five of its first drafts were wrong and the backend caught every one** — a fair verdict on the checksums. `subtotalMinor` is `Σ lineTotal` _after_ line discounts, with `discountMinor` a separate sale-level figure; a **tender cannot be refunded more than it took** (the split sale's Rs 1,390 line could not go back as cash, only to the card that paid it — the rule that stops a card sale being cashed out at the counter); the closing denomination breakdown must **sum to the figure claimed**, so you cannot assert a total you did not count out; and **Rs 200 is not a circulating LKR note**, which the shift service knows. One of my assertions was itself wrong in an instructive way: it expected a cashier's refund attempt to be `401/403`, but the answer is **422 with the person named** — the PIN was accepted and the _role_ refused, which is a different and better thing to say. The drawer arithmetic was wrong too, subtracting a card refund from the cash drawer; the reconciliation caught it, which is exactly what a reconciliation is for                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-08-29 | M4-10       | —           | **Sizing measured rather than estimated, and the earlier estimate was wrong.** `pg_column_size` against the live cloud puts a sale at **4–7.5 KB including indexes** — indexes are the bulk of it, not the rows, and the `§G` figure of “100–200 MB/year/shop” was too optimistic because it guessed row widths and underweighted index overhead. Neon's free 500 MB therefore lasts **~5 years at 50 sales/day, 2–3 years at 100, and under a year at 300+** — fine for a corner shop, not for a supermarket. `DEPLOYMENT.md` gains a **Going paid** section: Render Starter $7 + Neon Launch $19 = **$26/month for the whole estate, not per client**, since one Neon project holds every tenant (which is what `tenant_id` and `V206`'s `(tenant_id, client_uuid)` uniqueness are for). The reason to pay is **not storage** — it is Neon's **7-day point-in-time restore** against free's 24 hours: a shop ringing on Thursday about Tuesday is past the free window. Recorded alongside it, because it is the kind of thing that outlives its reason: **delete the keep-warm cron when Render stops sleeping**, or it becomes a monitor that pages you about a healthy service. Also answered: a client wanting **till + back office without the owner console** needs no fork — `apps/terminal` imports nothing from `apps/console`, the desktop profile loads none of the 18 `@Profile("cloud")` classes, a sale commits with `db-cloud` stopped (verified, HTTP 201), and an entitlement cache that has never been filled means **no restrictions recorded** rather than nothing allowed. It is a plan tier via the existing `owner_console` flag, not a build variant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-08-29 | GATE-M4     | **PASS**    | **The owner saw today's takings on a phone, on mobile data, from outside the shop's network.** Rs 9,309.00, matching the till, with the sync time beside it — the figure and its freshness shown together, because a cloud reading an outbox is only ever as current as the last drain. M4 closes. What the gate actually proves is the inversion of §A end to end: the sale was final on the shop PC before anything touched the network, the outbox carried it to Singapore on its own schedule, and the phone read a cloud that had never been on the critical path of anything. The intervening misconfiguration made the point better than a clean run would have — three sales were taken, printed and finalised while the cloud was unreachable, then drained in a single batch when a valid token appeared, with no operator action and no reconciliation. Infrastructure: Neon + Render + Vercel, all free tier, all `ap-southeast-1`, one Docker image portable to ECS or ACA by environment variables alone. **The honest cost: six failed deploys, five of them caused by my own diagnosis.** The bug never moved — Neon's hostname carries a `.c-3.` segment and omitting it returns `28P01 password authentication failed`, an _auth_ error for a _hostname_ fault. I advised removing it on the strength of `nslookup` appearing to confirm the shorter form, not knowing Neon serves wildcard DNS, which makes that lookup incapable of failing; four further theories were then built on that non-measurement. One `psql` from outside Render settled it in a single command. **The rule worth keeping: measure the connection before theorising about it, and never validate a Neon hostname with DNS**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-08-29 | M4-10       | —           | **A sale rung on a terminal reached the cloud on its own.** The full path ran for the first time against real infrastructure: local Postgres → outbox → `POST /api/sync/batch` → Neon in Singapore. Cloud now holds 2 sales (Rs 9,309.00), 4 items, 2 payments, 4 stock movements, carrying invoice numbers `KND-T1-000428/9` issued from the till's **local** block — the per-terminal numbering of §A holding up over a real network. All 97 outbox rows acked, none unacked. Two things were learned the hard way and both are properties, not bugs. **`setx /M` does not reach an already-open shell**, and often not a newly-opened one either until sign-out, because Windows Terminal caches the environment — the reliable move for a dev session is setting `$env:` in the same window that launches the backend; the machine-level variable still matters on a shop PC, where the backend starts as a service after a reboot. And **a till with no token is silent by design**: `SyncWorker` logs `Nothing to sync with: no lumora.sync.token is configured` and returns _before_ opening a connection, so the symptom is `pending: N` with `lastAttemptAt: null` and no error anywhere — diagnosable only by reading those two fields together. The intervening state was the offline guarantee working exactly as intended: three sales taken, printed and finalised while the cloud was unreachable, then drained in one batch the moment a valid token appeared. Nothing lost, no operator action, no reconciliation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-08-29 | M4-10       | —           | **The console signs in from the internet.** Vercel → Render → Neon works end to end: owner authenticates, `GET /api/console/today` returns `200`, and the whole read path is exercised against real infrastructure rather than a test double. Tenant 1 "Pilot Shop One" created through `POST /api/platform/tenants` — one call producing tenant, trial licence to 2027-08-29, the owner's console login and the till token. The first login attempt failed and the failure was **correct**: the platform admin password was used on the owner's screen, and the two are deliberately different credential kinds against different endpoints (`/api/platform/auth/login` vs `/api/console/auth/login`). Worth remembering when onboarding a real shop — the error message says the account does not match, which is true and reads like a typo. Bootstrap password rotated on first sight because it had been pasted into a transcript; old password and old session token both verified dead. Still outstanding before Gate M4 can be attempted: `LUMORA_PLATFORM_BOOTSTRAP_EMAIL` still set on Render, no keep-warm cron, and **no till provisioned — so `lastSyncAt` is null and the console shows zero takings, which is honest rather than broken**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-08-29 | M4-10       | —           | Cloud hosting provisioned: **Neon** (`ep-dawn-credit-azh6unu8`, `lumora_cloud`, ap-southeast-1) + **Render** (`storex-cloud.onrender.com`, Singapore, Docker, free) + Vercel for the console. Live and verified: 12 migrations applied (`V1`, `V200`–`V210`, 28 tables, no `V100`+ so the cloud tier ran), `/actuator/health/readiness` `UP`, `/api/*` 401 with `/actuator/health` 200. Platform admin bootstrapped, password **rotated immediately** because the generated one was logged where it could be read; the old password and its session token both verified dead afterwards. Six failed deploys preceded this and **five of them were self-inflicted diagnosis**, which is the part worth recording. The real bug was one thing throughout: **Neon's hostname carries a `.c-3.` segment and it is not optional** — dropping it produces `SQL State 28P01 · password authentication failed`, an _authentication_ error for a _hostname_ fault, so every instinct points at the credential and the credential is fine. The advice to strip `.c-3.` came from `nslookup` appearing to confirm the shorter form — but **Neon serves wildcard DNS**, so a lookup answers for any name of roughly the right shape and cannot fail. A test that cannot fail was treated as evidence, and four further theories (single-quoted username in the error text, IPv6 vs Render's egress, `channelBinding=require`, a newline-mangled `JAVA_TOOL_OPTIONS`) were stacked on top of it. What actually settled it in one command was connecting with `psql` from outside Render and proving the same credentials succeed with `.c-3.` and fail with `28P01` without it. **The rule: measure the connection before theorising about it, and never validate a Neon hostname with DNS.** Two smaller true findings survive from the wreckage: Neon's connection string uses libpq's `channel_binding`, which the JDBC driver silently ignores (its spelling is `channelBinding`, and it is unsupported here anyway — drop it and keep `sslmode=require`); and recreating a database or branch **changes the endpoint id**, so a stale hostname keeps resolving and keeps returning `28P01`. Also confirmed: Render env vars **replace** the image's `JAVA_TOOL_OPTIONS` rather than appending, and a newline pasted into that field truncates the flag list. Remaining: `LUMORA_CONSOLE_ORIGINS` is still unset (preflight returns 403), the console is not yet deployed, the keep-warm cron is not created, and no till is provisioned — so **Gate M4 is not attemptable yet**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
