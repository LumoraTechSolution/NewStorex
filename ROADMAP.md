...
> **Current position:** **M2 complete — 12 of 12 done. Two gates are outstanding, and both need a person.** M1 landed the sale path (18 tasks); M2 landed the accountability layer on top of it: shifts with a blind denomination count (`M2-01`–`M2-04`, `V107`), signed cash movements (`M2-05`), receipt-linked returns with a manager-PIN gate, per-line reasons, partial-return apportionment and refunds locked to the original tender (`M2-06`–`M2-10`, `V108`), a printable local Z-report (`M2-11`), and all three new aggregates syncing to the cloud (`M2-12`, `V203`). **D1 is settled** — the variance threshold is per-tenant, defaulting to LKR 100.00. Verified against running software: 122 backend tests, 132 domain, 56 terminal, and 11 Playwright specs driving the **real Electron window**, all with zero pointer events; the sync loop confirmed in `lumora_cloud`. Gate M0 passed 2026-08-12. **One behaviour change to know about: the till now refuses to sell without an open shift** — see §G for why that costs §A nothing. **Next: attempt Gate M1 and Gate M2**, both by a human — 20 consecutive sales without a mouse, and a serious attempt to refund without a receipt or to a tender the sale never took. Then M3. Verification gaps still open, all in §G: `M1-14`'s serial transport has never written to a physical printer (none is attached here; TCP is the only transport proven end to end), and `M1-11`'s tender overlay still wants the human keyboard-only pass Gate M1 will supply. **`§D` remains stale in a way that matters: the IRD invoice-format mandate is already in force and `M5-09` is therefore late, not upcoming — see the 2026-08-20 rows in §G.**
